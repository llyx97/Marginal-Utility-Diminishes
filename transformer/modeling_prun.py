...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None):
        ...
        if self.config.emb_hidden_dim == -1:
            ...
            words_embeddings = self.word_embeddings1(input_ids)
            words_embeddings = self.word_embeddings2(words_embeddings)
...
class BertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if config.prun_hidden_size % config.num_attention_heads != 0:
            raise ValueError(
                "The hidden size (%d) is not a multiple of the number of attention "
                "heads (%d)" % (config.hidden_size, config.num_attention_heads))
        self.num_attention_heads = config.num_attention_heads
        self.attention_head_size = int(
            config.prun_hidden_size / config.num_attention_heads)
    ...
    def __init__(self, config, intermediate_size=-1):
        ...
        if intermediate_size < 0:
            self.dense = nn.Linear(
                config.hidden_size, config.prun_intermediate_size)
        else:
...
class BertOutput(nn.Module):
    def __init__(self, config, intermediate_size=-1):
        ...
        if intermediate_size < 0:
            self.dense = nn.Linear(
                config.prun_intermediate_size, config.hidden_size)
        else:
...
class IntermediaBertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, teacher_outputs, teacher_weight, teacher_prob, train_teacher_att):
        ...
        all_encoder_layers.append(hidden_states)
        return all_encoder_layers, all_encoder_atts, all_teacher_atts, all_feature_norms
...
class IntermediaBertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, teacher_outputs, teacher_weight, teacher_prob, token_type_ids=None, attention_mask=None,
        ...
        encoded_layers, layer_atts, teacher_atts, feature_norms = self.encoder(embedding_output, extended_attention_mask,
                                                                      teacher_outputs, teacher_weight, teacher_prob, train_teacher_att)
        ...
        return encoded_layers, layer_atts, pooled_output, teacher_atts, feature_norms
...
class IntermediaModelForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config, num_labels, fit_size=768):
        ...
        self.num_labels = num_labels
        self.bert = IntermediaBertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(self, input_ids, teacher_outputs, teacher_weight, teacher_prob, token_type_ids=None, attention_mask=None,
        ...
        sequence_output, att_output, pooled_output, teacher_atts, feature_norms = self.bert(input_ids, teacher_outputs, teacher_weight, teacher_prob, token_type_ids,
        ...
        return logits, att_output, sequence_output, teacher_atts, feature_norms

...
class BertAttention(nn.Module):
    ...
    def forward(
        ...
        if (self.self is not None) and (self.output is not None):
...
class BertIntermediate(nn.Module):
    ...
    def forward(self, hidden_states, ffn_mask=None):
        hidden_states = self.dense(hidden_states)
        if ffn_mask is not None:
            hidden_states = hidden_states * ffn_mask
...
class BertLayer(nn.Module):
    def __init__(self, config):
        ...
        self.pruned_ffns = set()

    def prune_ffns(self, ffns_to_prune):
        if len(ffns_to_prune) == 0:
            return
        if self.intermediate is None:
            return

        all_pruned = self.pruned_ffns.union(ffns_to_prune)
        ffn_size = self.intermediate.dense.weight.size(0)
        if len(all_pruned) == ffn_size:
            self.intermediate = None
            self.output = None
            # Update hyper params and store pruned ffns
            self.pruned_ffns = all_pruned
            return

        mask = torch.ones(ffn_size)
        ffns_to_prune = set(ffns_to_prune) - self.pruned_ffns
        for ffn in ffns_to_prune:
            # Compute how many pruned neurons are before the neuron and move the index accordingly
            ffn = ffn - sum(1 if f < ffn else 0 for f in self.pruned_ffns)
            mask[ffn] = 0
        ...
        # Update hyper params and store pruned heads
        self.pruned_ffns = self.pruned_ffns.union(ffns_to_prune)
    ...
    def forward(
        ...
        intermediate_output = self.intermediate(attention_output, ffn_mask)
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_outputs = layer_module(
                hidden_states, attention_mask, head_mask[i], encoder_hidden_states, encoder_attention_mask, ffn_mask=ffn_mask[i]
            )
...
class BertModel(BertPreTrainedModel):
    ...
    def _prune_ffns(self, ffns_to_prune):
        """ Prunes FFN of the model.
            ffns_to_prune: dict of {layer_num: list of FFN neurons to prune in this layer}
            See base class PreTrainedModel
        """
        for layer, ffns in ffns_to_prune.items():
            self.encoder.layer[layer].prune_ffns(ffns)
    ...
    Examples::
        ...
        ffn_mask = self.get_ffn_mask(ffn_mask, self.config.num_hidden_layers)
